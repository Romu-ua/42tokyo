...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/03/24 15:09:16 by hyamamot          #+#    #+#             */
/*   Updated: 2025/03/24 15:09:19 by hyamamot         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main(int argc, char **argv, char **envp)
{
	char	*cmd1;
	char	*cmd2;
	char	*options1[] = {"-l", NULL};
	char	*options2[] = {"file1", NULL};
	pid_t	pid;
	int		status;
	...
	cmd1 = cmd(argv[2], envp);
	cmd2 = cmd(argv[3], envp);

	// コマンドを取得できている
	printf("%s\n", cmd1);
	printf("%s\n", cmd2);

	// execve(cmd1, options1, envp);
	// execve(cmd2, options2, envp);
	...
	{
		// 子プロセス
		if (execve(cmd1, options1, envp) == -1)
			perror("execve");
		return (1);
	}
	else
